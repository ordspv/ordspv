...
- **Tier A, chain-derivable (trustless-capable):** `/r/blockhash[/h]`,
  ...
  `/content/<id>`, `/r/metadata/<id>`. These are computable from headers plus raw
  transactions; a verify-gateway SHOULD serve them from chain data.
- **Tier B, index-dependent:** `/r/inscription/<id>` (number, sat, charms…),
  `/r/children/*`, `/r/parents/*` (reverse edges / location validation),
  `/r/sat/*`, `/r/utxo/*`. These require an ord index; responses are trusted-index
  claims and MUST NOT carry `x-ord-verification`.
...
remains recommended (every 200 here is immutable): hotlinking pressure is what took
ordinals.com down (ord#3873).
...
- Streaming vs buffering: proxy passthrough streams bodies above the
  per-entry cap. VERIFIED responses are buffered, since a merkle proof
